// ChatGPT Content Script - Captures conversations from ChatGPT
// Monitors chat interactions and enables task linking

import browser from 'webextension-polyfill';

console.log('🤖 TaskWeave ChatGPT injector loaded');
...
function init() {
  console.log('🔧 Initializing ChatGPT injector...');
  
  // Wait for ChatGPT to load
  waitForChatGPT().then(() => {
    console.log('✅ ChatGPT detected, setting up capture');
    setupCaptureButton();
    observeMessages();
  });
}
...
async function waitForChatGPT(): Promise<void> {
  return new Promise((resolve) => {
    const checkInterval = setInterval(() => {
      // Check for ChatGPT main container
...
// Extract conversation from ChatGPT UI
function extractConversation(): Message[] {
  const messages: Message[] = [];

  try {
    // ChatGPT message selectors (may need updates as UI changes)
    const messageContainers = document.querySelectorAll('[data-message-author-role]') ||
                             document.querySelectorAll('.group.w-full') ||
                             document.querySelectorAll('[class*="Message"]');

    messageContainers.forEach((container) => {
      const role = container.getAttribute('data-message-author-role') ||
                  (container.querySelector('.whitespace-pre-wrap') ? 'user' : 'assistant');
      
      const contentEl = container.querySelector('.whitespace-pre-wrap') ||
    ...
    // Alternative extraction method (ChatGPT structure changes frequently)
    if (messages.length === 0) {
      const alternativeMessages = document.querySelectorAll('div[data-testid^="conversation-turn"]');
      alternativeMessages.forEach((msg) => {
        const isUser = msg.querySelector('[data-message-author-role="user"]');
...
function showCaptureDialog(messages: Message[]) {
  ...
  dialog.innerHTML = `
    ...
    <p style="margin: 0 0 16px 0; color: #666;">
      ${messages.length} messages will be captured
    </p>
